...
export function transition(before,action){
 ...
 if(action.type==='forfeit'&&!['memory','finished'].includes(s.phase)){eliminate(s,p,events);events.find(e=>e.type==='eliminate').reason='forfeit';}
  ...
  const lock=s.players[p].locked;
  if(lock&&lock.card===s.players[p].slots[i]?.id&&lock.against===s.discard.at(-1)?.id)throw Error('That penalty cannot go straight back');
  ...
  if(ok){s.players[p].slots[i]=null;know(s,p,i,null);s.discard.push(c);s.discardOwners[c.id]=p;}
   ...
   // A penalty may not go back onto the very card it just uncovered, or a miss
   // against a repeated rank would undo itself. Anything discarded on top later
   // buries that card and the penalty becomes an ordinary card again.
   s.players[p].locked=s.discard.length?{card:penalty.id,against:s.discard.at(-1).id}:null;
  ...
  if(s.players[p].slots.every(c=>c===null)){
   if(s.caller===null){
   ...
   }else if(s.remaining.includes(p)){
    // A final round is already under way and this player just matched away their
    // last card, so they have nothing left to play. Left in the rotation, they
    // would eventually become the active player with an empty hand and no legal
    // move — drop them from `remaining` now, and if it was already their turn,
    // end it immediately instead of waiting on a turn they cannot take.
    const wasActive=p===s.active;
    s.remaining=s.remaining.filter(q=>q!==p);
    if(wasActive){
     if(!s.remaining.length){s.phase='lastCall';events.push({type:'lastCall'});}
     else{s.active=s.remaining[0];s.phase='ready';}
    }
   }
  }
  ...
  // An absent player's turn is passed over, not forfeited: skipping costs the
  // turn only, and the multiplayer layer decides when repeated misses remove them.
  else if(action.type==='pass'){events.push({type:'pass',p});endTurn(s,events);}
