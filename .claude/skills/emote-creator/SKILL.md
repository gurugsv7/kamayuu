---
name: emote-creator
description: Turns a source video or PNG frame sequence of a character animation into a fully working in-game emote for the Kamayuu/lotus-buzzer card game — a transparent sprite-sheet PNG, a CSS steps() animation, a trigger button, and both local and networked-multiplayer wiring. Use this whenever the user wants to add a new emote/reaction/sticker animation to the game, mentions "emote", "sticker", "reaction animation", hands over a green-screen clip or a folder of animation frames for the table, or asks to repeat the "haha" cat emote pipeline for a new expression. Covers the exact ffmpeg chroma-keying, sprite-sheet assembly, CSS math, React state, and dual-copy multiplayer-rules wiring this repo needs — follow it end to end rather than improvising a new approach, since several naive approaches (black-background colorkey, animated WebP, VP9-alpha video) were already tried here and failed.
---

# Emote Creator (Kamayuu / lotus-buzzer)
