...
function buzz(s,events){s.caller=s.active;s.remaining=Array.from({length:s.players.length-1},(_,i)=>(s.active+i+1)%s.players.length).filter(p=>!s.players[p].eliminated);events.push({type:'buzz',p:s.active});if(!s.remaining.length){final(s,events);return;}s.active=s.remaining[0];s.phase='ready';}
...
export function transition(before,action){
 ...
 if(action.type==='forfeit'&&!['memory','finished'].includes(s.phase)){eliminate(s,p,events);events.find(e=>e.type==='eliminate').reason='forfeit';}
  ...
  else if(action.type==='buzz'&&s.caller===null)buzz(s,events);
...
export function assertState(s){
 ...
 if(new Set(s.remaining).size!==s.remaining.length||s.remaining.includes(s.caller))throw Error('Invalid final turns');
