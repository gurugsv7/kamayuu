...
export default function Home(){
 ...
 else if(e.type==='buzz'){setCaption('FINAL ROUND');setMessage(e.keepTurn?`${name} buzzed. ${e.p===0?'You still play this turn':'They still play this turn'} — then one last turn each.`:`${name} buzzed. Everyone else gets one last turn.`);sound('buzz');const buz=table.current?.querySelector('.buzzer') as HTMLElement;await animate(buz,[{transform:'translateY(0)'},{transform:'translateY(8px)',offset:.2},{transform:'translateY(-2px)',offset:.6},{transform:'translateY(0)'}],500,token);await wait(900,token);}
