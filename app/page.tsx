...
export default function Home(){
 ...
 async function start(){cancel();if(remote.current){try{remote.current.close();}catch{}remote.current=null;setLobby(null);lobbyRef.current=null;setOnline(null);setPresence([]);setConnection('');setClock(null);}memorized.current=false;audio.current?.unlock();setMenu(false);setPanel(null);const token=epoch.current;locked.current=true;setBusy(true);const {state,events}=createGame(config.current);commit(state);setCaption('THE LANTERN TABLE');setMessage('A fresh hand. A clean slate.');await paint();
 ...
 else if(e.type==='buzz'){setCaption('FINAL ROUND');setMessage(`${name} buzzed. Everyone else gets one last turn.`);sound('buzz');const buz=table.current?.querySelector('.buzzer') as HTMLElement;await animate(buz,[{transform:'translateY(0)'},{transform:'translateY(8px)',offset:.2},{transform:'translateY(-2px)',offset:.6},{transform:'translateY(0)'}],500,token);await wait(900,token);}
 ...
 async function playPacket(packet:any){
  ...
  if(!next){cancel();game.current=null;setS(null);setMemory(false);return;}
 ...
 async function connect(command:'create'|'join'|'recover',extra:any={}){
  ...
  try{
   ...
   if(!remote.current)remote.current=new MultiplayerService((pk:any)=>packetHandler.current(pk),setConnection,setPresence);
 ...
 const before=game.current;if(locked.current||!before||['finished','memory'].includes(before.phase)||paused.current||(!ai&&before.players[0].eliminated)||(!ai&&action.type!=='match'&&before.active!==0)||(!ai&&action.type==='finish'))return false;
 if(before.remote){
 ...
 if(action.type==='match'&&!ai)action={...action,player:0};let plan:any;try{plan=transition(before,action);}catch{return false;}
 ...
 {gate==='ready'&&menu&&(
    <HomeScreen
      ...
      onPractice={(diff) => {
        setSettings((x) => ({ ...x, difficulty: diff }));
        audio.current?.unlock();
        start();
      }}
